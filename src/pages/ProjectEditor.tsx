...
import {
  ArrowLeft, Info, Code2, Link2, ExternalLink, Coffee,
  BrainCircuit, ImagePlus, Save, Sparkles, Plus, X,
  Settings2, CloudUpload, Wand2, Camera,
} from "lucide-react";
import { useAuth } from "../context/AuthContext";

const API = "http://localhost:5000";

const TECH_SUGGESTIONS = ["React", "Next.js", "TypeScript", "Node.js", "Flutter", "Django", "PostgreSQL", "Tailwind", "GraphQL", "Redis", "SCSS", "Angular", "Socket.IO", "Prisma"];
...
export default function ProjectEditor() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/projects`, {
      ...
      .then((data) => {
        ...
        if (!p) { setSaveError(`Project not found (id: ${id}).`); return; }
  ...
  async function handleAutoScreenshot() {
    if (!demoUrl.trim()) { setSaveError("Add a Demo URL first to auto-screenshot."); return; }
    setSaveError("");
    setScreenshotting(true);
    try {
      const res = await fetch(`${API}/api/media/screenshot`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ url: demoUrl.trim(), fullPage: false }),
      });
      const data = await res.json();
      if (!res.ok) { setSaveError(data.error || "Screenshot failed"); return; }
  ...
  async function handleAiGenerate() {
    if (!aiContext.trim()) { setSaveError("Paste your raw context in the AI & Notes section first."); return; }
    if (!title.trim()) { setSaveError("Add a project title first."); return; }
    setSaveError("");
    setAiLoading(true);
    try {
      const res = await fetch(`${API}/api/projects/generate-descriptions`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ title, aiContext, techStack, role, status, architectureNotes: archNotes }),
      });
      const data = await res.json();
      if (!res.ok) { setSaveError(data.message || "AI generation failed"); return; }
      ...
      if (typeof data.data.featured === "boolean") setFeatured(data.data.featured);
    } catch {
      setSaveError("Could not reach the server.");
    } finally {
      setAiLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setSaveError(data.error || data.message || `Failed to ${isEdit ? "update" : "create"} project.`);
        return;
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-on-surface-variant text-sm mt-1">
              {isEdit ? `Editing "${title || "…"}"` : "Initialize a new venture in the VAY Ecosystem."}
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <section className="bg-surface-container-high rounded-xl p-8 border border-outline-variant/10">
            ...
            <div className="space-y-6">
              <Field label="Project Title">
                <input type="text" placeholder="e.g. Pendu Chat" value={title}
              ...
              <Field label="Slug">
                <div className="relative">
                  <input type="text" placeholder="pendu-chat" value={slug}
              ...
              <Field label="Short Description">
                <textarea rows={2} placeholder="Brief summary for list views..." value={shortDesc}
              ...
              <Field label="Full Description">
                <textarea rows={6} placeholder="Detailed project breakdown — or fill via AI Generate below…"
          ...
          <section className="bg-surface-container-high rounded-xl p-8 border border-outline-variant/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <Field label="Role">
                <input type="text" placeholder="Solo / Lead / Contributor" value={role}
              ...
              <div className="flex items-center justify-between pt-6">
                ...
                <button type="button" onClick={() => setFeatured((f) => !f)}
                  ...
                  <span className={`inline-block h-5 w-5 rounded-full transition-transform duration-200 ${featured ? "translate-x-5 bg-primary" : "translate-x-0.5 bg-outline"}`} />
                </button>
              </div>
            </div>
          </section>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <section className="bg-surface-container-high rounded-xl p-8 border border-outline-variant/10">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Tech Stack</label>
                <div className="flex flex-wrap gap-2">
                  {techStack.map((tech) => (
                    <span key={tech} className="px-2 py-1 bg-primary/10 text-primary text-[10px] font-bold rounded-lg flex items-center gap-1 border border-primary/20">
                      {tech}
                      <button type="button" onClick={() => removeTech(tech)} className="hover:text-white transition-colors"><X size={10} /></button>
                    </span>
                  ))}
                </div>
                <div className="flex gap-2">
                  <input type="text" value={techInput} onChange={(e) => setTechInput(e.target.value)}
                    onKeyDown={handleTechKeyDown} placeholder="Add tech, press Enter…"
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {TECH_SUGGESTIONS.filter((t) => !techStack.find((x) => x.toLowerCase() === t.toLowerCase())).slice(0, 7).map((t) => (
              ...
              <UrlField label="GitHub URL" icon={<Link2 size={14} />} placeholder="https://github.com/vay/..." value={githubUrl} onChange={setGithubUrl} />
              <UrlField label="Demo URL" icon={<ExternalLink size={14} />} placeholder="https://..." value={demoUrl} onChange={setDemoUrl} />
              <UrlField label="Buy Me Coffee URL" icon={<Coffee size={14} />} placeholder="https://buymeacoffee.com/..." value={coffeeUrl} onChange={setCoffeeUrl} />

              {/* Partner */}
              <div className="pt-4 border-t border-outline-variant/10">
                <p className="text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-4">
                  Collaborator <span className="text-outline font-normal normal-case tracking-normal ml-1">(optional)</span>
                </p>
                <div className="space-y-4">
                  <Field label="Partner Name">
                    <input type="text" placeholder="e.g. John Doe" value={partnerName} onChange={(e) => setPartnerName(e.target.value)}
                  ...
                  <UrlField label="Partner GitHub" icon={<Link2 size={14} />} placeholder="https://github.com/partner" value={partnerGithub} onChange={setPartnerGithub} />
                  <UrlField label="Partner Portfolio" icon={<ExternalLink size={14} />} placeholder="https://partner.dev" value={partnerPortfolio} onChange={setPartnerPortfolio} />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-surface-container-high rounded-xl p-8 border border-outline-variant/10">
            ...
            <div className="space-y-6">
              <Field label="Architecture Notes">
                <textarea rows={3} placeholder="Core stack logic, scalability notes..." value={archNotes}
                  onChange={(e) => setArchNotes(e.target.value)}
                  className="w-full bg-transparent border-0 border-b border-outline-variant/20 focus:border-primary focus:outline-none text-on-surface py-3 transition-all placeholder:text-outline/40 resize-none text-sm" />
              </Field>
              <Field label="Raw context for AI generation">
                <textarea rows={5} placeholder="Paste raw notes, bullet points, features, decisions — anything. The AI will generate both the short and full description from this."
                  value={aiContext} onChange={(e) => setAiContext(e.target.value)}
                  className="w-full bg-transparent border-0 border-b border-outline-variant/20 focus:border-primary focus:outline-none text-on-surface py-3 transition-all placeholder:text-outline/40 resize-none text-sm" />
              </Field>
              <button type="button" onClick={handleAiGenerate} disabled={aiLoading || !aiContext.trim()}
              ...
              {aiLoading && (
                <p className="text-[10px] text-on-surface-variant text-center">Calling OpenRouter — this takes 5–15 seconds…</p>
              )}
            </div>
          </section>
          ...
          <section className="bg-surface-container-high rounded-xl p-8 border border-outline-variant/10">
            ...
            <div className="flex items-center justify-between mb-4">
              <label className="text-xs font-bold uppercase tracking-wider text-on-surface-variant block">Screenshots</label>
              <button type="button" onClick={handleAutoScreenshot} disabled={screenshotting || !demoUrl.trim()}
            ...
            <label
              ...
              <CloudUpload size={36} className={`transition-colors mb-4 ${dragOver ? "text-primary" : "text-outline"}`} />
              {uploading ? (
                <p className="text-sm font-medium text-primary">Uploading…</p>
              ) : (
                <>
                  <p className="text-sm font-medium text-on-surface-variant text-center">
                    Drag and drop screenshots here or <span className="text-primary font-bold">browse</span>
                  </p>
                  <p className="text-[10px] text-outline mt-2">PNG, JPG or WebP · Max 10MB each</p>
                </>
              )}
            </label>
            ...
            {screenshots.length > 0 && (
              <div className="mt-4 grid grid-cols-2 gap-3">
                {screenshots.map((url, i) => (
                  <div key={url} className="relative group rounded-lg overflow-hidden border border-outline-variant/20">
                    <img src={url} alt={`screenshot ${i + 1}`} className="w-full h-28 object-cover" />
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <label className="text-xs font-bold uppercase tracking-wider text-on-surface-variant block">{label}</label>
      {children}
    </div>
  );
}

function UrlField({ label, icon, placeholder, value, onChange }: {
